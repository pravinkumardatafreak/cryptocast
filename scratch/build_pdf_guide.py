...
def generate_pdf():
    ...
    code_modules = [
        ...
        ("src/train_model_pytorch.py — Model Architectures &amp; Training Loop",
         [
            ...
            "• Lines 131–158: CNN1D class — 3 Conv1d layers (64→64→32) with causal padding, AdaptiveAvgPool1d, Linear head → 3 outputs.",
            "• Lines 161–182: RNNModel class — 2 stacked SimpleRNN layers (64→32), last timestep hidden state → Linear head → 3 outputs.",
            "• Lines 185–210: LSTMModel class — 3 stacked LSTM layers (128→64→32) with gated memory highway → Linear head → 3 outputs.",
            "• Lines 213–248: TransformerModel — Linear projection to d_model=256, 2-layer TransformerEncoder (4 heads), AvgPool → head → 3.",
            "• Lines 251–276: RevIN class — forward(x, 'norm') subtracts mean, divides by std; forward(x, 'denorm') reverses the operation.",
            "• Lines 279–323: PatchTSTModel — RevIN → unfold() creates 5 patches of size 12 → value+position embeddings → 3-layer encoder → head.",
            "• Lines 345–364: DirectionalMSELoss — MSE + alpha * mean(ReLU(-y_pred * sign(y_true))), alpha=0.30.",
        ...
        ("src/step4_wfv.py — Walk-Forward Validation",
         [
