...
def test_alpha_strategy():
    ...
    for t in range(len(test_df)):
        ...
        # ENTRY: Risk-On + PatchTST Bullish + Mon/Tue + Weekly Open Discount
        ...
        if buy_condition and not in_position:
            ...
            target_price = pred_7d_price
            in_position = True
            trades += 1

        elif in_position:
            # EXIT CONDITIONS:
            # 1. Take Profit: Reached PatchTST predicted target price
            # 2. Stop Loss: Dropped -2.5% below entry
            # 3. Time Exit: End of week (Sunday)
            tp_hit = current_price >= target_price
            sl_hit = current_price <= entry_price * 0.975
            time_exit = dt.dayofweek == 6

            if tp_hit or sl_hit or time_exit or not is_risk_on:
    ...
    print("======================================================================")
    print("      ALPHA STRATEGY WITH TAKE-PROFIT & STOP-LOSS SIMULATION         ")
    print("======================================================================")
